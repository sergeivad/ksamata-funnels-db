...
- **`funnels`** — one row per funnel: identity FKs (source/product/contractor —
  **`product_id` и `contractor_id` тут кэш осей, а не сами оси**: истину
  ...
  шаг предсписка (Phase 16, умолчание 1).
...
**Набор получают не все воронки** — с 02.09.2026 (Phase 16) предсписок стал
свойством воронки: колонка `funnels.has_predspisok`, галка на карточке. При
нуле сценарий не материализуется вовсе — не «пустой набор», а отсутствующий,
и `scenarioViews` не выводит строку. Разница не косметическая: оси и маркер
типа неудаляемы (`isIdentity`), поэтому оверрайд «убрать тег этапа» оставил бы
пять тегов идентичности; погасить сценарий можно только признаком.
Оверрайды при снятой галке **не стираются** — вернул галку, набор вернулся.

Признак на **воронке**, а не на типе (в отличие от `has_time`), и это замер:
все 34 воронки с предпиской имеют тип `АВ Автоворонка`, но 31 автоворонка
предсписка не имеет, а продукт и статус не делят вовсе. Тип — условие
необходимое, но не достаточное.

Числа — замеры по снимкам реестра, а не свойство модели, и растут каждую
неделю: 25.08 — 16 воронок из 76, 01.09 — 33 из 77, 02.09 — 34 из 78.
Синхронизации с GetCourse нет: галку ставит человек там же, где решает завести
предложение.
...
**С классом 14 он пересекается, и это не задвоение.** Свежее предложение
этапа, по которому заказов ещё нет, у воронки со снятым признаком попадает на
оба листа: 14 говорит «заказов нет, кандидат в архив», 17 — «признак не
поднят». Оба верны, и действия по ним противоположны, поэтому человеку нужны
оба. Гасить 14 нельзя: настоящий архивный кандидат спрятался бы ровно тогда,
когда галку поднимут, а предложение так и не выстрелит. Закреплено тестом,
чтобы не «починили» как дубль.

**Не путать «класс 17» с «Phase 17»** — цифра совпала случайно, сущности
разные: класс 17 это находка аудита (`tools/audit/findings.py`), Phase 17 —
миграция осевых FK-колонок. Оба заведены 02.09.2026 разными сессиями, номера
в своих пространствах шли подряд.
...
потому что наборы сравниваются с ним дословно. Менялся не подход, а реестр.
До августа 2026 GetCourse писал этап `Предписок`, без «с» — своя опечатка,
которую мы обязаны были повторять; между снимками реестра 30.07 и 01.09 он её
исправил. Замер: 30.07 старое написание несли 14 предложений и ни одного —
новое; 01.09 старого нет вовсе, а новое несут 34, причём те же 14 живы, у них
просто сменился тег. **Сверка при этом не ломается, а тихо даёт ноль
совпадений** — то есть выглядит как «расхождений нет»; именно так расхождение и
прожило месяц. Человеческая подпись строки — «Предсписок»
(`scenarioViews` в `tag-scenarios.ts`) — теперь совпала с тегом случайно, а не
по правилу: подпись называет шаг воронки, тег повторяет реестр. Легаси-тег
`предсписок` — **третья**, посторонняя сущность: другой тег, автоматически ни к
чему не сводится, и теперь он отличается от тега этапа только префиксом и
регистром. Одна строка написания живёт в двух местах, на двух языках —
`PHASE14_STAGE_TAG`
...
править обе, согласие сторон держит
[app/tests/stage-tag-spelling.test.ts](app/tests/stage-tag-spelling.test.ts).
**Одной правкой констант существующую базу не починить**: сид строки шаблона у
фазы 14 разовый, за маркером, — переписывает её Phase 15.
...
- **Phase 15** — тег этапа предсписка переписывается с `АВ Этап: Предписок` на
  `АВ Этап: Предсписок` вслед за GetCourse, исправившим свою опечатку в августе
  2026 (см. раздел про теги). Отдельная фаза, а не правка констант, ровно по
  устройству фазы 14: строку шаблона та сеет **разово, за маркером**, поэтому на
  уже промигрированной базе смена константы до шаблона не доезжает — а
  материализация читает `tag_templates`, не константу, и раскладывала бы старое
  написание по 460+ строкам `funnel_tags` при каждом старте. Три шага, все
  безусловные: строка шаблона, оверрайды (сегодня таких нет ни одного, но
  переживший фазу оверрайд всплыл бы в наборе одной воронки) и имя в
  справочнике `tags`. Последний переносит все материализованные строки разом —
  они ссылаются на `tag_id`, а не на текст, так что `funnel_tags` не правится
  вовсе. Развилка «слить, а не переименовать» — для базы, где новое имя успело
  появиться раньше (шаблон правили через `/api/tag-templates`, и
  `resyncAllFunnels` завёл вторую строку справочника). Идёт **после** фазы 14 и
  сама себя гасит: как только старого написания в базе нет, работы не находит.
- **Phase 16** — предсписок становится свойством воронки: колонка
  `funnels.has_predspisok` (`NOT NULL DEFAULT 1`), разовый бэкфилл нулями и
  безусловный снос наборов `predspisok` у воронок с нулём. Фаза 14 раздавала
  набор ВСЕМ воронкам, объявив сценарий частью модели, а в реестре предложений
  GetCourse этап заведён у меньшинства — 34 из 78 на замере 02.09.2026. Список
  из 44 F-кодов зашит в `migrate-phase16-data.ts` и **датирован**: это снимок
  реестра, как `TIMELESS_FUNNEL_TYPES` у фазы 12, а не правило; дописывать в
  него новые коды не нужно — признак живёт на карточке. Ключ — `front_code`
  (уникален, непуст у всех 78, переживает перенос между базами, в отличие от
  `id` и `num`). Бэкфилл срабатывает **только в прогон, заводящий колонку**:
  `NOT NULL DEFAULT 1` не отличает «ещё не решали» от «решили, что предсписок
  есть», и безусловный бэкфилл затирал бы галку человека при каждом старте —
  дословно грабли фазы 12. Третий шаг безусловен и потому
  самовосстанавливающийся, из цепочки фазу не убираем. Идёт **после** фазы 14 —
  и та тоже знает про признак: её материализация иначе возвращала бы наборы 44
  воронкам при каждом старте, а эта фаза тут же их сносила. Колонки у фазы 14
