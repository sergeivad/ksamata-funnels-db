...
| [superpowers/specs/2026-08-17-sheet-links-design.md](superpowers/specs/2026-08-17-sheet-links-design.md) | Сверка таблицы «Воронки ссылки» с блоками `tariffs`/`applications` (`tools/sheet-links/`) | **Active** — инструмент собран и отчёт получен на живых данных, читает только. Заливка результата в прод (через его HTTP API, не в репозиторную базу) — отдельная задача, остаётся открытой |
