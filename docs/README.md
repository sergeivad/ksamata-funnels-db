...
| [plans/2026-08-24-leak-new-funnels.md](plans/2026-08-24-leak-new-funnels.md) | Три новые воронки в ЛИК + правило замены легаси-фильтров | **Active** — **начинать с него любую сессию по ЛИК.** Заведены `f66`, `f68`, `f87` (реестр 63 → 66), комнаты `f66`/`f68` записаны в прод, `f95` перенесена с прода в репозиторную базу. Снято прежнее неизвестное: история при замене правил **не переписывается**, пересчёт идёт только от `effectiveFrom`. 28.08 через интерфейс заведена `f96` (реестр 66 → 67) — интерфейс ЛИК кликается целиком, `confirm()` при активации больше нет. Замена легаси-фильтров **сделана 25.08**: 20 воронок из 22 переведены на АВ-набор с `effectiveFrom 2026-07-01` (замер: 0 ₽ потерь, +86 290 ₽), блокировку сняла проба на `f15` — ноль потерянных ключей. Остались `f25` (дата проставлена, теги прежние) и `f16` (не трогаем по решению владельца) |
