...
| [plans/2026-08-24-leak-new-funnels.md](plans/2026-08-24-leak-new-funnels.md) | Три новые воронки в ЛИК + правило замены легаси-фильтров | **Active** — **начинать с него любую сессию по ЛИК.** Заведены `f66`, `f68`, `f87` (реестр 63 → 66), комнаты `f66`/`f68` записаны в прод, `f95` перенесена с прода в репозиторную базу. Снято прежнее неизвестное: история при замене правил **не переписывается**, пересчёт идёт только от `effectiveFrom`. 28.08 через интерфейс заведена `f96` (реестр 66 → 67) — интерфейс ЛИК кликается целиком, `confirm()` при активации больше нет. Замена легаси-фильтров **завершена**: 25.08 переведены 20 воронок, 28.08 дозавершена пропущенная `f25` — итого **21 из 22** с `effectiveFrom 2026-07-01` (замер: 0 ₽ потерь, +86 290 ₽); `f16` не трогаем, она архивная. Опасение «пересохранение теряет вебинарные настройки» закрыто собственным замером на `f25`: ноль потерянных ключей во всех девяти rule-JSON, `attendanceRuleJson` байт в байт. ⚠️ При пересохранении набора у существующей воронки открывается нативный `confirm()` — OK нажимает человек. Найдена причина бедного `attendanceRuleJson` у шести вебинарных воронок (`f66`, `f59`, `f60`, `f61`, `f81`, `f16`): не заполнена отдельная сущность «Настройки вебинара», и пересохранением набора это не чинится — нужен перенос конфига вендором |
