...
> **Поправка по итогам живой проверки (18.09.2026).** Пункт «проверяются все
> адреса воронки, включая выключенные» ниже — **устарел**. Живой прогон на
> `f37` дал пять падений, из которых четыре пришлись на группу `links`:
> админские страницы GetCourse, требующие сессии и отвечающие `403` всегда.
> Правильное правило: проверяются адреса с `enabled = 1` **или** с включённым
> дефолтом их группы. Выключенность у черновика (воронка не активна) и
> выключенность группы (так решил человек) — разные вещи, и различать их
> обязательно. Подробности — в спеке, раздел 4.
