...
Поднять превью (`MONITOR_ENABLED=false` в `.env.local`, иначе фоновая проверка нагадит в отслеживаемую базу) и пройти пять адресов.
...
**Форма без кода живьём НЕ проверяется** (решение 2026-08-13). После Task 1
бескодовых воронок не осталось, а снимать код в админке ради проверки значило
бы править отслеживаемую базу — это спорит с правилом «не оставлять `.db`
изменённым». Ветку закрывают юнит-тесты: `funnelHref({frontCode:'', id})`
в Task 3 и разбор чистых цифр в `parseFunnelRef`.
