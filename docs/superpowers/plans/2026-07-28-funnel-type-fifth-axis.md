...
- Данные воронок и теги меняются только через `createFunnel` / `updateFunnel`
  или API. Raw SQL по живой базе запрещён.
...
Из `app/`: `MONITOR_ENABLED=false npm run dev`, открыть http://localhost:3000

Проверить: (1) на карточке воронки появился селектор «Тип воронки» с четырьмя
значениями и пустым вариантом; (2) выбор значения и сохранение меняют список
тегов внизу карточки — маркер меняется во всех четырёх сценариях;
(3) в списке воронок чипа нет, пока тип «АВ Автоворонка».

После проверки остановить сервер и вернуть БД:
