 ...
 * должны уже существовать (Фаза 8). На проде это делает docker-entrypoint.sh
 ...
 *
 * СТАТУС: применён и локально, и на проде — 2026-07-29, обе базы дают
 * 60 / 11 / 1 / 0, шаблон очищен в обеих. Прод правил не этот скрипт (он
 * пишет в базу напрямую, а прод правится только через API), а его продовый
 * близнец `set-funnel-types-prod-2026-07-29.ts`, для которого локальная база
 * после этого прогона и служит источником правды.
 *
