    ...
    <header className="sticky top-0 z-40 border-b border-[var(--line-soft)] bg-[var(--card)]/95 backdrop-blur">
      <div className="mx-auto flex h-14 max-w-[1120px] items-center gap-4 px-6">
        ...
        <button
          ...
          onClick={createDraft}
          disabled={creating}
          className="ml-auto inline-flex items-center gap-1.5 rounded-[8px] bg-[var(--orange)] px-3.5 py-2 text-[13px] font-semibold text-white transition hover:opacity-90 disabled:opacity-60"
        >
          <Plus size={15} />
          {creating ? 'Создание…' : 'Новая воронка'}
        </button>
      </div>
    </header>
  );
