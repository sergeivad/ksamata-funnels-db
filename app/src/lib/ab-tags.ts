...
// ─── Legacy generator (kept transitionally) ────────────────────────────────
// `axesToTagNames` is superseded by `computeTagSet` above, but `funnels.ts`
// (`syncAvTags`) and `FunnelIdentity.tsx` still call it directly. It is
// migrated off in Tasks 5 and 9; Task 9 deletes this function once its last
// caller is gone. Kept verbatim (unchanged) from its prior version.
