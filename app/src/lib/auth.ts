...
const PUBLIC_GET_PATTERNS: RegExp[] = [
  ...
  /^\/help$/,
  // …и её скриншоты. Открыть страницу, но не её картинки — значит показать
  // анониму инструкцию с битыми иллюстрациями: `matcher` мидлвары исключает
  // только `_next/static`, а всё из `public/` проходит через авторизацию как
  // обычная страница и анониму отвечает редиректом на вход. В dev это не
  // видно — там учётки не настроены и пускают всех; ловится только на
  // продовой сборке. Список узкий намеренно: ровно каталог справки и webp.
  /^\/help\/[a-z0-9-]+\.webp$/,
  /^\/api\/funnels$/,
  /^\/api\/funnels\/\d+$/,
  /^\/api\/funnels\/\d+\/days$/,
