...
def test_payment_type_is_a_property_of_the_order_not_of_the_funnel():
    """«АВ Тип оплаты» база не хранит намеренно — решение владельца 2026-07-31.

    Ось появилась в GetCourse на пяти предложениях (все `draft`, все со
    значением `оверлей`, все на ЖИВО-суставах). Владелец: оверлей — способ
    оплаты конкретного ЗАКАЗА, а не признак, различающий воронки. Значит
    воронку она не описывает и в словаре базы ей места нет — как мессенджеру
    (свойство заказа) и линейке (выводится из продукта).
    """
    assert is_external_tag('АВ Тип оплаты: оверлей')
